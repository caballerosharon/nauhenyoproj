<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white">
    <!-- New styled orange header -->
    <div class="bg-orange-500 shadow-md">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        <h1 class="text-2xl font-bold text-white">BFP Firefighter Profile</h1>
        <button 
          @click="logout" 
          class="bg-white text-orange-500 hover:bg-orange-100 font-bold py-2 px-4 rounded transition duration-300"
        >
          Log Out
        </button>
      </div>
    ...
    <div class="p-8">
      <div class="max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-xl overflow-hidden">
        <div class="p-6">
          <h2 class="text-3xl font-bold mb-6 text-center">Firefighter Details</h2>
          <div v-if="currentFirefighter" class="space-y-6">
            ...
            <div v-if="!isEditing" class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="space-y-2">
                ...
                <p class="font-semibold">{{ currentFirefighter.status }}</p>
              </div>
            </div>
          ...
          <div v-if="currentFirefighter" class="mt-8">
            ...
            <div v-if="assignedReports.length > 0" class="space-y-4">
              <div v-for="report in assignedReports" :key="report.id" class="bg-gray-700 p-4 rounded-lg">
                ...
                <div v-if="report.status !== 'Completed'" class="mt-2">
                  <button 
                ...
                <p v-else class="text-green-400 mt-2">Completed</p>
              </div>
            </div>
            <p v-else class="text-gray-400">No reports assigned at the moment.</p>
          </div>
        </div>
      </div>
    </div>
...
const completeReport = async (report) => {
  try {
    ...
    await fireReportStore.updateReport(report.id, {
      status: 'Completed'
    });

    // Update firefighter status to Available
    if (currentFirefighter.value) {
      await firefighterStore.updateFirefighterStatus(currentFirefighter.value.id, 'Available');
      currentFirefighter.value.status = 'Available';
    }
