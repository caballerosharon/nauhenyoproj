      ...
      <div class="flex-1 overflow-y-auto p-8">
        <form @submit.prevent="submitReport" class="bg-white shadow-md rounded-lg p-6 space-y-6">
          ...
          <div class="flex items-center">
            ...
            <input type="file" id="photos" multiple @change="handleFileUpload" accept="image/*"
                   class="w-3/4 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
          </div>
          ...
          <div v-if="selectedFiles.length > 0" class="flex flex-wrap gap-2 mt-2">
            <div v-for="(file, index) in selectedFiles" :key="index" class="relative">
              <img :src="getFilePreview(file)" alt="Selected image" class="w-20 h-20 object-cover rounded" />
              <button @click.prevent="removeFile(index)" class="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center">
                <X class="w-3 h-3" />
              </button>
            </div>
          </div>
