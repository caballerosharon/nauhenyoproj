  ...
  directionsService.route(request, (result, status) => {
    if (status === google.maps.DirectionsStatus.OK) {
      ...
      if (status === google.maps.DirectionsStatus.ZERO_RESULTS) {
        ...
        alert("Unable to retrieve directions. Please try again later.");
      }
    }
  });
};
  ...
  try {
    ...
    fireSnapshot.forEach((doc) => {
      ...
      if (report.coordinates) {
        ...
        const location = report.location || 'Not specified';
    ...
    fireReportStore.setLocationCounts(locationCounts);
  } catch (error) {
...
const highestFireRateArea = computed(() => {
  const locationCounts = fireReportStore.locationCounts;
  if (!locationCounts || Object.keys(locationCounts).length === 0) {
    return null;
  }

  const highestArea = Object.entries(locationCounts).reduce((a, b) => a[1] > b[1] ? a : b);
