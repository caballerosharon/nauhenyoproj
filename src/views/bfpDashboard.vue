      ...
      <div class="bg-[#070b0d] text-white p-4 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4 relative">
          ...
          <button @click="toggleNotificationModal" class="relative p-1 rounded-full hover:bg-gray-700 transition-colors duration-200">
            <Bell class="h-6 w-6" />
            <span v-if="fireReportStore.unreadNotifications > 0" class="absolute top-0 right-0 bg-red-500 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">
      ...
      <div v-if="isNotificationModalOpen" class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
        <div class="bg-white rounded-lg shadow-lg max-w-lg w-full p-6">
          ...
          <ul class="space-y-4 max-h-96 overflow-y-auto">
            <li v-for="notification in fireReportStore.notifications" :key="notification.id" class="flex justify-between items-center p-2 hover:bg-gray-100 rounded">
              <div>
              ...
              <span v-if="!notification.read" class="text-xs font-bold text-red-500">Unread</span>
            </li>
            <li v-if="fireReportStore.notifications.length === 0" class="text-sm text-gray-500">No notifications available.</li>
          </ul>
...
const markAllAsRead = () => {
  fireReportStore.notifications.forEach(notif => {
    fireReportStore.markNotificationAsRead(notif.id);
  });
};
...
const recentIncidents = computed(() => fireReportStore.recentFireReports);
...
const incidentTypesData = computed(() => {
  const typeCounts = fireReportStore.fireReports.reduce((acc, report) => {
...
const monthlyTrendsData = computed(() => {
  ...
  const monthlyCounts = fireReportStore.fireReports.reduce((acc, report) => {
...
onMounted(async () => {
  console.log('BFP Admin Dashboard component mounted');
  await fireReportStore.fetchFireReports();
});

onUnmounted(() => {
  // Clean up any listeners or subscriptions if necessary
});
</script>
